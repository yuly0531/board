...
function ajax(
	...
	try{
		...
		$.ajax({
				...
				alert("isEmpty 함수 호출시 예외발생"+ e.message )
...
//MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM
		// 매개변수로 들어오는 Jquery객체가 관리하는 태그에 입력된 
		//문자의 앞뒤 공백을 제거하고 다시 넣어주기 
		//
//MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM
...
function inputAfterBlankDel(obj){
	try{
			if(obj.length==0){
				alert("inputAfterEmptyDel 함수 호출시"+"1번째 매개변수로 들어오는 JQuery객체가 관리하는 태그가 없습니다.")
				return;
			}
			if(obj.is(":checkbox")||obj.is(":radio")){
				alert("inputAfterEmptyDel 함수 호출시"
							+"1번째 매개변수로 들어오는 JQuery객체가 관리하는 태그는"
							+"체크박스도 아니고 라디오버튼도 아니어야 합니다.")
				return;
			}
			
			var val=obj.val();
			
			if(typeof(val)=='string'){obj.val(obj.val().trim());}
			}
			catch(e){
				alert("inputAfterEmptyDel 함수 호출시 예외발생"+ e.message )
			}
}
